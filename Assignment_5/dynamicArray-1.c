...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
  /* FIXME: Write This */
    addDynArr(heap, val);
    int newIndex = sizeDynArr(heap) -1;
    while(newIndex != 0) {
        int parent = (newIndex - 1) / 2;
        if (compare(getDynArr(heap, newIndex),getDynArr(heap, parent)) == -1) {
            swapDynArr(heap, newIndex, parent);
            newIndex = parent;
        } else return;
    }
}
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
  /* FIXME: Write this */
    ...
    //max = is the index of the last element in the array
    //pos = is the index of root
    int leftChild = (2 * pos) + 1; int rightChild = (2 * pos) + 2;
    //case#1 = root at (sub)tree has two children
    
    
    if (rightChild < max) {
        int smallerofTwo = _smallerIndexHeap(heap, leftChild, rightChild, compare);
        if (compare(getDynArr(heap, pos), getDynArr(heap,smallerofTwo)) == 1) {
            swapDynArr(heap, pos, smallerofTwo);
            _adjustHeap(heap, max, smallerofTwo, compare);
        }
    }
    //case 2 (this will only happen if the root parent hasonly one child who is a leaf node)
