...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    clearVisited(g);
        ...
        cirListDeque *knownReach = malloc(sizeof(cirListDeque));
        initCirListDeque(knownReach);
        addFrontCirListDeque(knownReach, source);
        while (!isEmptyCirListDeque(knownReach)) {
            Vertex *Vertex = malloc(sizeof(Vertex));
            Vertex = frontCirListDeque(knownReach);
            removeFrontCirListDeque(knownReach);
            Vertex->isVisited = 1;
            if (Vertex == destination)
                return 1;

            for (int i = 0; i < Vertex->numNeighbors; i++){
                if (Vertex->neighbors[i]->isVisited != 1)
                    addFrontCirListDeque(knownReach, Vertex->neighbors[i]);
                }
        }
    /*
        removeAllCirListDeque(knownReach);
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
    ...
    cirListDeque *knownReach = malloc(sizeof(cirListDeque));
    initCirListDeque(knownReach);
    addBackCirListDeque(knownReach, source);
    Vertex *Vertex = malloc(sizeof(Vertex));
    ...
    while (!isEmptyCirListDeque(knownReach)) {
        ...
        for (int i = 0; i < Vertex->numNeighbors; i++){
            if (Vertex->neighbors[i]->isVisited != 1)
                addBackCirListDeque(knownReach, Vertex->neighbors[i]);
        }
    }
