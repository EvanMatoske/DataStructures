    ...
    if (containsKey(hashTable, word) == 0)
        {
            ...
            if (letter == 'Y'){
                ...
                printf("'%s' has been added to the dictionary!", word);
            }
        }
    else {
        printf("That word exists!\n");
    }
...
void loadDictionary(FILE* file, struct hashMap* ht)
{
  /* You will write this*/
    ...
    char* inputWord;
    while ((inputWord = getWord(file)))
    {
        //printf("%s ", inputWord);
        
        int val = containsKey(ht, inputWord);
        if (val == 1)
        {
            int value;
            value = *atMap(ht, inputWord);
            value++;
            insertMap(ht, inputWord, value);
        }
        
        else (insertMap(ht, inputWord, 1));
    }
}
