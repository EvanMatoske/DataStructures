...
int main (int argc, const char * argv[]) {
    ...
    printf("For the input file: ");
    printf("\n");
    while ((inputWord = getWord(fileptr)))
    {
        printf("%s ", inputWord);
        
        int val = containsKey(hashTable, inputWord);
        if (val == 1)
        {
            int value;
            value = *atMap(hashTable, inputWord);
            value++;
            insertMap(hashTable, inputWord, value);
        }
         
        else (insertMap(hashTable, inputWord, 1));
    }
    //printMap(hashTable);
    /*
    printf("\n");
    ...
    {
        hashLink *Bucket;
        Bucket = hashTable->table[i];
        while (Bucket){
            printf("%d,%s, %d\n", Bucket->value, Bucket->key, i);
            Bucket = Bucket->next;
        }
    }
    */
